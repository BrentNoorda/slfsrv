...
func mustBeARawSelfServingSourceFile(filespec string) (exeFileData []byte,err error) {
    ...
    if fileOrDirExists,err = ssutil.FileOrDirExists(filespec); err != nil { return exeFileData,err }
    if isDir,err = ssutil.IsDir(filespec); err != nil { return exeFileData,err }
    ...
    if exeFileData,err = ioutil.ReadFile(filespec); err != nil { return exeFileData,err }
...
func Compile(exeOutPath string,replaceOK bool,exeInPath string,rootPath string,initialUrl string,verbose bool) (err error) {
    ...
    if _,err = mustBeARawSelfServingSourceFile(exeInPath); err != nil { return err }

    // output must be file name, not a directory
    var isDir bool
    if isDir,err = ssutil.IsDir(exeOutPath); err != nil { return err }
    if isDir {
    ...
    if !replaceOK {
        ...
        if fileOrDirExists,err = ssutil.FileOrDirExists(exeOutPath); err != nil { return err }
        if fileOrDirExists {
    ...
    if zipContents,err = bundle.BundleDirectory(rootPath,initialUrl); err != nil { return err }

    // append zipContents to buffer
    var exeFileBuf *bytes.Buffer = bytes.NewBuffer(exeFileData)
    if _,err = exeFileBuf.Write(zipContents.Bytes()); err != nil { return err }
