...
func ReadFile(reader *zip.Reader,filespec string) ([]byte,error) {
    ...
    for _, f = range reader.File {
        if f.Name == filespec {
            ...
            if rc,err = f.Open(); err != nil { return data,err }
...
func Reader(zipFilespec string,offsetByte0 int) (reader *zip.Reader,initialUrl string,err error) {
    ...
    if f,err = os.Open(zipFilespec); err != nil { return }

    // determine total size of file zipFilespec
    var fi os.FileInfo
    if fi,err = f.Stat(); err != nil { return }
    fileSize = fi.Size()

    var readerAt selfservingReaderAt = selfservingReaderAt{ offset:int64(offsetByte0), f:f }

    if reader,err = zip.NewReader(readerAt,fileSize-readerAt.offset); err != nil { return }

    // check that the version string is correct, and pull out initialUrl
    var data []byte
    if data,err = ReadFile(reader,"slfsrv"); err != nil { return }
    /* not checking the slfsrv version yet */

    if data,err = ReadFile(reader,"initialUrl"); err != nil { return }
...
func BundleDirectory(rootPath string,initialUrl string) (buf *bytes.Buffer,err error) {
    ...
    var isDir bool
    if isDir,err = ssutil.IsDir(rootPath); err != nil { return }
    if !isDir {
    ...
    walkFunc := func(path string, info os.FileInfo, err error) error {
        if err == nil {
            if !info.IsDir() {
                ...
                if f, err = w.Create(filepath.ToSlash(relativePath)); err != nil { return err }

                if fileData,err = ioutil.ReadFile(path); err != nil { return err }

                if _,err = f.Write(fileData); err != nil { return err }

            }
        }
        return err
    }
    if err = filepath.Walk(rootPath,walkFunc); err != nil { return }
...
func Bundle(outFilespec string,replaceOK bool,rootPath string,initialUrl string,verbose bool) (err error) {
    ...
    var isDir bool
    if isDir,err = ssutil.IsDir(outFilespec); err != nil { return err }
    if isDir {
    ...
    if !replaceOK {
        ...
        if fileOrDirExists,err = ssutil.FileOrDirExists(outFilespec); err != nil { return err }
        if fileOrDirExists {
    ...
    if zipContents,err = BundleDirectory(rootPath,initialUrl); err != nil { return err }
